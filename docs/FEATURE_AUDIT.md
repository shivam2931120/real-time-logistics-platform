...
Suggested sequence: durable route publishing, payment reconciliation, operational alerts and idempotent offline driver actions are complete, so next is refund/settlement review, service territories and customer address preferences. These can be built with the existing stack; hosted capacity and third-party services may still have costs or usage limits.
