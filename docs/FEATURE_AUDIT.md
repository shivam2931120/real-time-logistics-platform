...
| Maps                    | MapLibre, configurable OpenFreeMap style, address search through Nominatim/Photon gateways, road geometry through OSRM gateway, location controls, layers, clustering, stale GPS indicators | External tiles/search/routing can fail or be rate-limited. Road geometry is not a traffic-aware ETA service.                                                             |
...
| Payments                | Razorpay checkout, verification/webhook integration and demo adapter                                                                                                                        | Razorpay order-to-delivery mapping is in process memory. Durable mapping, idempotency and reconciliation are priority hardening before payment-dependent production use. |
