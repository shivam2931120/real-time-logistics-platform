...
| Maps                    | MapLibre, configurable OpenFreeMap style, address search through Nominatim/Photon gateways, road geometry through OSRM gateway, location controls, layers, clustering, stale GPS indicators, capped route concurrency and retry states | External tiles/search/routing can fail or be rate-limited. OSRM remains geometry-only; route-run ETAs use the published sequence and persisted GPS until a traffic provider is configured.                                                             |
...
| Routes                  | Capacity-aware optimization, selected stops, drag reordering, distance/duration estimates, map preview, durable versioned route runs, draft/publish lifecycle, driver-visible published plans, and route-run-aware customer ETAs that include the latest driver GPS fix and preceding planned stops | Provider traffic feeds and multi-depot constraints remain future work. |
