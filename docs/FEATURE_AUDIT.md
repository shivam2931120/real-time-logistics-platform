...
| Delivery operations     | Creation, search/status filters, assignment, driver acceptance/rejection, status transitions, delivery windows, exceptions, bounded CSV preview and idempotent batch creation             | Larger imports still require a queued worker and downloadable failure artifacts.                                                                                         |
...
| Geofences               | Configurable circular geofences, arrival/departure events, dwell analytics, persisted tenant service polygons and drop-off validation | Restricted-area rules and polygon editing directly on the map remain future work. |
...
| Payments                | Razorpay checkout, verification/webhook integration, durable provider references, atomic payment/order writes, webhook deduplication, tenant-scoped reconciliation, and bounded provider settlement CSV review with explicit accept/reject decisions | Automatic refund execution and provider API settlement polling remain future work. |
...
| Analytics and reports   | 7/30/90-day summaries, completion/on-time/payment rates, revenue, risk, status/priority/driver/zone/geofence analysis, CSV exports and estimated cost-per-delivery metrics             | Add historical event-backed trends, saved report filters and actual fuel/driver-cost integrations. Direct kilometres and cost estimates are not actual driven costs.      |
...
3. **Refund and provider settlement imports.** Settlement CSV import and review are available; next is provider API polling plus immutable refund/chargeback ledger corrections. Never auto-correct an order from an unreviewed provider row.
...
7. **Validated bulk import and batch dispatch.** CSV preview, row-level errors, duplicate detection, territory checks and bounded idempotent creation are available; next is queued processing, dry-run capacity checks and downloadable failure reports.
8. **Performance comparisons and cost per stop.** Period comparisons and configurable estimated cost per km/stop are available. Next is event-backed cohorts, actual fuel/driver-cost integrations and separate booked/collected/refunded money, estimated/actual distance and on-time denominator definitions.

Suggested sequence: durable route publishing, payment reconciliation, operational alerts, idempotent offline driver actions, settlement review, service territories, customer address preferences, the first SLA inbox slice and bounded bulk import are complete, so next is queued batch dispatch and cost-per-stop analytics. These can be built with the existing stack; hosted capacity and third-party services may still have costs or usage limits.
