...
export function configureSockets(io: Server){
 io.use((socket,next)=>{const token=String(socket.handshake.auth.token||'');if(process.env.AUTH_MODE==='clerk'){void verifyToken(token,{secretKey:process.env.CLERK_SECRET_KEY}).then(resolveClerkUser).then(user=>{socket.data.user=user;next()}).catch(()=>next(new Error('unauthorized')));return;}try{const c=jwt.verify(token,process.env.JWT_SECRET||'routepulse-local-development-only',{issuer:'routepulse',audience:'routepulse-web'}) as jwt.JwtPayload;const user=users.find(u=>u.id===c.sub);if(!user)throw new Error();socket.data.user=user;next()}catch{next(new Error('unauthorized'))}});
