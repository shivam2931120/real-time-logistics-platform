...
export function authenticate(req: Request, res: Response, next: NextFunction) {
  ...
  if (process.env.AUTH_MODE === 'clerk' && demoAuthEnabled()) {
    try {
      const claims = jwt.verify(raw, secret(), { issuer: 'routepulse', audience: 'routepulse-web' }) as jwt.JwtPayload;
      if (claims.demo === true && typeof claims.role === 'string') {
        req.user = demoUserForRole(claims.role as Role);
        next();
        return;
      }
    } catch {
      // This is not a sandbox token; continue with normal Clerk verification.
    }
  }
