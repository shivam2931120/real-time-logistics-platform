...
export function notificationMode(){ return process.env.REDIS_URL ? 'bullmq' : 'inline-simulated'; }
...
export async function notify(order: Order, template: string) {
  ...
  if(!process.env.REDIS_URL){ const result=await deliverEmail(payload); return {...result,mode:result.status==='sent'?'smtp':'simulated'}; }
