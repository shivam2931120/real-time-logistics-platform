...
try {
  ...
  await client.query(
    ...
    ["00000000-0000-0000-0000-000000000001", "RoutePulse Demo"],
  );
  for (const user of users)
    await client.query(
      `INSERT INTO users(id,organization_id,email,name,role) VALUES($1,$2,$3,$4,$5::user_role) ON CONFLICT DO NOTHING`,
      [
        asUuid(user.id),
  ...
  for (const order of orders) {
    ...
    for (const e of order.events)
      await client.query(
        `INSERT INTO order_events(id,organization_id,order_id,type,message,actor_id,created_at) VALUES($1,$2,$3,$4,$5,$6,$7) ON CONFLICT DO NOTHING`,
        [
          asUuid(e.id),
          "00000000-0000-0000-0000-000000000001",
          asUuid(order.id),
  ...
  await client.query(
    `UPDATE orders SET customer_name=$1,customer_email=$2 WHERE id=$3 AND customer_email='vikram@example.com'`,
    ["Kabir Customer", "customer@routepulse.demo", asUuid("ord_1002")],
  );
  await client.query("COMMIT");
  console.info("RoutePulse demo data seeded");
} catch (e) {
