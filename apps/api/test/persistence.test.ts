...
import { asUuid, initializePersistence, persistOrder } from '../src/db/persistence.js';
...
describe.runIf(runDatabaseTests)('PostgreSQL persistence', () => {
  ...
  it('keeps tenant and entity IDs stable across a database reload', async () => {
    ...
    const order: Order = {
      ...
      createdAt,
      updatedAt: createdAt,
      events: [{ id: randomUUID(), type: 'created', message: 'Persistence test created', actorId: 'u_dispatch', createdAt }],
    };
    ...
    expect(reloaded).toMatchObject({ id: testOrderId, organizationId: 'org_demo', trackingCode: order.trackingCode });
